...
export function GuildSidebar() {
  ...
  const lastVisitedChannelByGuild = useAppStore(
    (s) => s.lastVisitedChannelByGuild,
  );
  ...
  return (
    <nav
      ...
      className="scrollbar-none flex flex-col items-center overflow-y-auto bg-[var(--bg-tertiary)] pb-3"
      style={{
        width: GUILD_SIDEBAR_WIDTH,
        paddingTop: "calc(var(--titlebar-inset) + 0.5rem)",
      }}
      aria-label="Guilds"
    >
      {/* Home button */}
      ...
      <button
        ...
        className="mt-2 flex h-11 w-11 items-center justify-center rounded-2xl bg-[var(--bg-secondary)] text-[var(--bg-accent)] transition-all duration-200 hover:rounded-xl hover:bg-[var(--bg-accent)] hover:text-[var(--text-on-accent)] hover:shadow-[var(--shadow-glow)]"
      >
        <svg
          className="h-5 w-5"
          viewBox="0 0 24 24"
